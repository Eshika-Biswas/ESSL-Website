      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className={`transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
            <div className="relative rounded-2xl border border-slate-200 bg-white backdrop-blur-xl overflow-hidden">
              {/* Chat Header */}
              <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
                ...
                <div>
                  <span className="text-sm font-medium text-slate-900">ESSL AI Advisor</span>
              ...
              <div className="p-6 space-y-4">
                ...
                {prompts.map((prompt, index) => (
                  ...
                  >
                    <MessageSquare className={`w-4 h-4 flex-shrink-0 ${activePrompt === index ? 'text-primary' : 'text-slate-400 group-hover:text-slate-700'} transition-colors`} />
                    <span className={`text-sm ${activePrompt === index ? 'text-primary font-medium' : 'text-slate-600 group-hover:text-slate-900'} transition-colors`}>
                      {prompt}
                    </span>
                    <ArrowRight className={`w-3.5 h-3.5 ml-auto flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all ${activePrompt === index ? '!opacity-100 text-primary' : 'text-muted'}`} />
                  </button>
                ))}
              </div>
